...
import dao.hybernateDAO.UserDAOH;
import entity.User;
...
public class RegistrationAndAuthController {
    ...
    public String authUser(ModelMap model, @RequestParam(value = "login", required = false) String login,
                           @RequestParam(value = "password", required = false) String password, HttpSession session) throws NoSuchAlgorithmException {
        User user = null;
        if (login != null) {
            if (FormValidator.checkLoginAndPassword(login, password) &&
                    ((user = daoh.getUserByLoginAndPass(login.toLowerCase(), password))) != null) {
